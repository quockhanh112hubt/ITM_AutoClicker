...
class MainWindow(QMainWindow):
    ...
    def _launch_self_update(self, downloaded_exe_path: str, info: UpdateInfo | None):
        ...
        script = "\r\n".join([
            ...
            "echo [%date% %time%] Deleting old exe >> \"%LOG%\"",
            "if exist \"%TARGET%\" (",
            "  for /l %%i in (1,1,10) do (",
            "    del /f /q \"%TARGET%\" >nul 2>&1",
            "    if not exist \"%TARGET%\" goto deleted_old",
            "    timeout /t 1 /nobreak >nul 2>&1",
            "  )",
            "  echo [%date% %time%] Failed to delete old exe >> \"%LOG%\"",
            "  exit /b 1",
            ")",
            ":deleted_old",
            "if exist \"%BACKUP%\" del /f /q \"%BACKUP%\" >nul 2>&1",
            "echo [%date% %time%] Renaming new exe >> \"%LOG%\"",
            "for /l %%i in (1,1,10) do (",
            "  ren \"%NEWFILE%\" \"%TARGET%\" >nul 2>&1",
            "  if exist \"%TARGET%\" goto renamed_ok",
            "  timeout /t 1 /nobreak >nul 2>&1",
            ")",
            "echo [%date% %time%] Failed to rename new exe >> \"%LOG%\"",
            "exit /b 1",
            ":renamed_ok",
            "set \"_PYI_ENTRY= \"",
